...
public:
   ...
   void MyLogMessage(const std::string & msg) const 
   {	
	   ...
	   FILE *f = fopen("serialmanager.log","a+");
	   if (f) 
	   {
		   fprintf(f, "   [%s]:%s", buf, msg.c_str());
		   fclose(f);
	   }
	   
   }

   void EnableDTR(bool dtr_enable) 
   {	   
#ifdef _WIN32
	   ...
	   if (ret == 0) {
		   ...
		   MyLogMessage(("Failed to change DTR: "
			   "EscapeCommFunction() returned Windows system error code " +
			   boost::lexical_cast<std::string>(err)).c_str());
		   CloseHandle(h);
	   } else {
		  // MyLogMessage("AsioClient::EnableDTR : ok\n");
	   }
#endif
   }
